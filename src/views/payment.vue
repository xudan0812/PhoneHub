    ...
    <div class="content page-order-payment">
      <div class="gray-box clear">
        ...
        <div class="box-inner order-info">
          ...
          <p class="payment-detail">Please complete the payment within <span>24 hours</span>, or the order will be cancelled.</p>
          <p class="payment-detail">We will ship within 72 hours after your payment.</p>
        </div>
        <div class="box-inner payment-checkout-panel clear">
          ...
          <span v-else class="is-pay">Paid</span>
          <span class="prices"> Total：   <em><span>$ </span>{{orderInfo.price + orderInfo.freight}}.00</em>   </span>
        </div>
      </div>
      ...
      <div class="confirm-detail">
        <div class="info-title">Shipping Infomation</div>
        <p class="info-detail">Name：{{orderInfo.receiveInfo.name}}</p>
        <p class="info-detail">Phone：{{orderInfo.receiveInfo.phone}}</p>
        <p class="info-detail">Address：{{orderInfo.receiveInfo.province}}{{orderInfo.receiveInfo.city}}{{orderInfo.receiveInfo.county}}{{orderInfo.receiveInfo.add}}</p>
      </div>
      <div class="confirm-detail">
        <div class="info-title">Billing Information</div>
        <p class="info-detail">Billing Type：Electronic</p>
        <p class="info-detail">Billing Title：{{orderInfo.invoiceName}}</p>
        <p class="info-detail">Info：Order Detail</p>
      </div>
      <div class="confirm-table-title clear">
        <span class="name fn-left">Order Information</span>
        <span class="subtotal fn-right">Subtotal</span>
        <span class="num fn-right">Count</span>
        <span class="price fn-right">Price</span>
      </div>
      <div class="confirm-goods-table">
        <div class="cart-items clear" v-for="good,index in orderInfo.goodsData">
          <div class="name fn-left hide-row">
            <div class="name-cell">
              <a href="javascript:;" :title="good.title +'（' + good.spec_json.show_name +'）'" target="_blank">{{good.title}}（{{good.spec_json.show_name}}）</a><br>
            </div>
          </div>
          <div class="subtotal fn-right">
            <div class="subtotal-cell">¥ {{good.price * good.count}}.00</div>
          </div>
          <div class="goods-num fn-right">{{good.count}}</div>
          <div class="price fn-right">¥ {{good.price}}.00</div>
        </div>
      </div>
      <div class="order-discount-line">
        <p> Total： <span>$ {{orderInfo.price}}.00</span> </p>
        <p> Shipping Fee： <span>+ $ {{orderInfo.freight}}.00</span> </p>
      </div>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      payNowHandle (orderId) {
        this.$store.commit('payNow', orderId)
        alert('Payment success for ' + (this.orderInfo.price + this.orderInfo.freight) + ' RMB')
