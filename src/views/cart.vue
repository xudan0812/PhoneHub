      ...
      <div class="cart-box">
        ...
        <div class="cart-inner">
          <div class="empty-label" v-if="count <= 0">
            <h3>Cart is empty</h3>
            <router-link to="/" class="link">Go to store?</router-link>
          </div>
          <div v-else>
                ...
                <div class="cart-top-items" v-for="item,index in carPanelData">
                  <div class="cart-items">
                    <div class="items-choose">
                      <span class="blue-checkbox-new" :class="{'checkbox-on':item.checked}" @click="checkGoodsHandle(item.sku_id)"></span>
                    </div>
                    ...
                    <div class="operation">
                      <a class="items-delete-btn" @click="delPanelDataHandle(item.sku_id)"></a>
                    </div>
                    <div class="subtotal">¥ {{item.price * item.count}}.00</div>
                    <div class="item-cols-num">
                      <div class="select js-select-quantity">
                        <span class="down" :class="{'down-disabled':item.count<=1}" @click="subPanelDataHandle(item.sku_id)">-</span>
                        <span class="num">{{item.count}}</span>
                        <span class="up" :class="{'up-disabled':item.count>=item.limit_num}"@click="plusPanelDataHandle(item.sku_id)">+</span>
                      </div>
                    </div>
          ...
          <div class="cart-bar-operation">
            <div>
              <div class="choose-all js-choose-all">
                <span class="blue-checkbox-new" :class="{'checkbox-on':allChecked}" @click="allCheckGoodsHandle(allChecked)"></span>
                Select All
              </div>
              <div class="delete-choose-goods" @click="delCheckedGoodsHandle()">Delete selected items</div>
            </div>
          </div>
          <div class="shipping">
            <div class="shipping-box">
              <div class="shipping-total shipping-num">
                <h4 class="">
                  <i>{{checkedCount}}</i> items selected
                </h4>
                <h5>
                  <i >{{count}}</i> items in total
                </h5>
              </div>
              <div class="shipping-total shipping-price">
                <h4 class="">
                  Total price：<span>￥</span><i >{{checkedPrice}}</i>
                </h4>
            ...
            <router-link tag="span" class="jianguo-blue-main-btn big-main-btn js-checkout" :class="{'disabled-btn': checkedCount<=0}" to="/checkout"><a>View Order</a></router-link>
          </div>
        </div>
      </div>
    </div>
...
<script>
  export default {
    ...
    methods: {
      delPanelDataHandle (id) {
        this.$store.commit('delCarPanelData', id)
      },
      plusPanelDataHandle (id) {
        this.$store.commit('plusCarPanelData', id)
      },
      subPanelDataHandle (id) {
