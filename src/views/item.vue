      ...
      <div class="item-box">
        <div class="gallery-wrapper">
          <div class="gallery">
            <div class="thumbnail">
              <ul>
                <li :class="{'on': index == imgIndex}" @click="tableImg(index)" v-for="img,index in itemInfo.ali_images">
        ...
        <div class="banner">
              ...
              <ul class="params-colors">
                <li :class="{'cur': color.id == $route.query.itemId}" v-for="color,index in itemInfo.sku_list">
                  <router-link :title="color.color" :to="{name:'Item', query:{itemId:color.id}}">
                    <img :src="'http://img01.smartisanos.cn/'+color.image+'20X20.jpg'"></router-link>
                </li>
              </ul>
            </div>
            <div class="sku-dynamic-params clear">
              <div class="params-name">Quantity</div>
              <div class="params-detail clear">
                <div class="item-num js-select-quantity">
                  <span class="down" :class="{'down-disabled':count<=1}" @click="subCount()">-</span>
                  <span class="num">{{count}}</span>
                  <span class="up" :class="{'up-disabled':count>=this.itemInfo.limit_num}" @click="addCount()">+</span>
                </div>
              </div>
            </div>
          </div>
          <div class="sku-status">
            <div class="cart-operation-wrapper clearfix">
              <span class="blue-title-btn js-add-cart" @click="addCarPanelHandle()"><a>Add to cart</a></span>
              <span class="gray-title-btn"><a>Buy now</a></span>
            </div>
          </div>
        </div>
      </div>
