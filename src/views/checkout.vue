    ...
    <div class="content page-order-checkout checkout">
              ...
              <ul class="address-item-list clear js-address-item-list">
                <li class="js-choose-address" :class="{'selected-address-item':receiveIndex == index}" @click="chooseReceive(index)" v-for="receive, index in receiveInfo">
                  ...
                  <div class="operation-section">
                    <span class="update-btn js-edit-address">Edit</span>
                    <span class="delete-btn js-delete-address">Delete</span>
                  </div>
                </li>
      ...
      <div class="gray-box">
        ...
        <div class="box-inner invoice-box js-invoice-box">
          <p class="invoice-detail"> Invoice type：electrical invoice </p>
          <div class="invoice-detail"> Invoice title：
            <div class="radio-box">
              <label>
                <input type="radio" class="hide" @click="changeInvoice(true)">
                <span class="blue-radio" :class="{'blue-radio-on':invoice.personal}"><a></a></span>  Proforma
              </label>
              <label>
                <input type="radio" class="hide" @click="changeInvoice(false)">
                <span class="blue-radio" :class="{'blue-radio-on':!invoice.personal}"><a></a></span>  Commercial
              </label>
            </div>
            <div class="module-form-row form-item fn-hide js-invoice-title" v-if="!invoice.personal">
              <div class="module-form-item-wrapper no-icon small-item">
                <i v-show="!invoice.name">Please enter a invoice title</i>
                <input type="text" class="js-verify" v-model="invoice.name">
                <div v-show="!invoice.name" class="verify-error">required</div>
              </div>
            </div>
          </div>
          <p class="invoice-detail">Invoice detail：Product List</p> <p class="invoice-label"> Electrical invoice can be used as evidence for after-sale service.</p>
        </div>
      </div>
      <div class="gray-box">
        ...
        <div class="box-inner">
          <div class="order-discount-line">
            <p> Subtotal Price： <span>¥ {{checkedPrice}}.00</span> </p>
            <p> Shipping fee： <span>+ ¥ {{freight}}.00</span> </p>
            <p class="discount-line js-discount-cash"> <em>Coupon</em>： <span> - 0 </span> </p>
          </div>
        </div>
        <div class="box-inner">
          <div class="last-payment clear">
            <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click="submitOrderHandle()"> <a>Submit Order</a> </span>
            <span class="prices fn-right">Total Price： <em>¥ {{checkedPrice + freight}}.00</em></span>
          </div>
        </div>
      </div>
    </div>
